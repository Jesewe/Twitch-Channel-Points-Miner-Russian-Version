...
def main():
    ...
    if not streamer_logins:
        print("Вы должны указать хотя бы одного стримера!")
        return
...
def read_streamer_logins():
    ...
    if len(sys.argv) <= 1:  # no command-line args (the first is always source file name)
        print("Введите имя стримера (или несколько через запятую). Первыми идут более важные стримеры.")
        twitch_streamers = input("Имена стримеров: ")
    ...
    for streamer_login in user_input:
        ...
        try:
            ...
            print(f"Стример {streamer_login} не существует")
        else:
